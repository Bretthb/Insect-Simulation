...
public class PoseUtils
{
    ...
    public static Pose steer(Pose pose, Point target, double maxTurnAngle)
    {
        ...
        return new Pose(pose.getX(), pose.getY(), pose.getHeading() + angleDiff);
    }
...

    {
        ...
        newX = point.getX() * Math.cos(pose.getHeading()) - point.getY()
                * Math.sin(pose.getHeading()) + pose.getX();
        newY = point.getX() * Math.sin(pose.getHeading()) + point.getY()
                * Math.cos(pose.getHeading()) + pose.getY();
        return new Point(newX, newY);
    }
    ...
    private static Point observedPosition(Point thisPoint, Point otherPoint)
    {
        ...
        if (Math.abs(thisPoint.getX() - otherPoint.getX()) > (.5 * SwarmConstants.SCREEN_WIDTH))
        {
        ...
        if (Math.abs(thisPoint.getY() - otherPoint.getY()) > (.5 * SwarmConstants.SCREEN_HEIGHT))
        {
